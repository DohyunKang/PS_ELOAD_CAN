                    ...
                    if (loopForm.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        /*try
                        {
                            using (SqlCeConnection connection = new SqlCeConnection("Data Source=" + _dbFilePath + ";Password= a1234!;"))
                            {
                                ...
                                string query = "SELECT COUNT(*) FROM [Program List] WHERE [Program Name] = @ProgramName AND [Loop] IS NOT NULL";
                                using (SqlCeCommand command = new SqlCeCommand(query, connection))
                                {
                                    command.Parameters.AddWithValue("@ProgramName", selectedProgramName);
                                    ...
                                    {
                                        // 이미 값이 있으면 업데이트
                                        string updateQuery = "UPDATE [Program List] SET [Loop] = @LoopValue WHERE [Program Name] = @ProgramName";
                                    ...
                                    {
                                        // 값이 없으면 새로 추가
                                        string insertQuery = "INSERT INTO [Program List] ([Program Name], [Loop]) VALUES (@ProgramName, @LoopValue)";
